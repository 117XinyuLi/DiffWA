...
class DiffusionProcessDDIM():
    def __init__(self, beta_1, beta_T, T, diffusion_fn, device, shape, eta, tau=1, encode_ratio=1, scheduling='uniform'):
        '''
    ...
    def _conditional_reverse_diffusion_step(self, x, mode='SSIM'):
        ...
        for _ in range(2):
            self.diffusion_fn.eval()
            x = torch.sqrt(self.alpha_bars[total_steps]) * x + torch.sqrt(
                1 - self.alpha_bars[total_steps]) * torch.randn_like(x)
            for prev_idx, idx in zip(range(total_steps, -2, -1), range(total_steps+1, -1, -1)):
                self.diffusion_fn.eval()
                noise = torch.zeros_like(x) if idx == 0 else torch.randn_like(x)

                if idx == 0:
                    alpha_bars_pre = torch.tensor(1, device=self.device)
                else:
                    alpha_bars_pre = self.alpha_bars[prev_idx]
                ...
                predict_epsilon = self.diffusion_fn(x, idx) - self.cond_fn(x_tilde, x, mode) * torch.sqrt(1-self.alpha_bars[idx]) * s_t

                sigma = self.sigmas[idx] * self.eta

                predicted_x0 = torch.sqrt(alpha_bars_pre) * (
                        x - torch.sqrt(1 - self.alpha_bars[idx]) * predict_epsilon) / torch.sqrt(self.alpha_bars[idx])
                direction_pointing_to_xt = torch.sqrt(1 - alpha_bars_pre - sigma ** 2) * predict_epsilon
                x = predicted_x0 + direction_pointing_to_xt + sigma * noise
