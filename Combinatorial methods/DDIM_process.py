...
class DiffusionProcessDDIM():
    ...
    def _one_reverse_diffusion_step(self, img, init_y):
        total_steps = N
    ...
    @torch.no_grad()
    def N_conditional_sampling(self, images, init_y, only_final=False):
        sampling_list = []

        final = None
        for sample in self._one_reverse_diffusion_step(images, init_y):
            final = sample
            if not only_final:
    ...
    def cond_fn(self, images, images_adv):
    ...
    def _conditional_reverse_diffusion_step(self, x, init_y):
        img = x.clone()
        total_steps = N
        self.diffusion_fn.eval()
        x = init_y
        ...
        for prev_idx, idx in zip(reversed(diffusion_process[:-1]), reversed(diffusion_process[1:])):
            ...
            predict_epsilon = self.diffusion_fn(x=x, img=img, idx=idx) + self.cond_fn(x_tilde, x) * s_t * torch.sqrt(1-self.alpha_bars[idx])
    ...
    @torch.no_grad()
    def N_distance_guidance_sampling(self, images, init_y, only_final=False):

        self.diffusion_fn.eval()
        final = None
        final_list = []
        for sample in self._conditional_reverse_diffusion_step(images, init_y):
            final = sample
            if not only_final:
    ...
    def _one_conditional_reverse_diffusion_step(self, img):
    ...
    def conditional_sampling(self, images, only_final=False):
        ...
        final = None
        for sample in self._one_conditional_reverse_diffusion_step(images):
            final = sample
            if not only_final:
                sampling_list.append(final)
    ...
    def _distance_guidance_reverse_diffusion_step(self, x):
        img = x.clone()
    ...
    def distance_guidance_sampling(self, images, only_final=False):
        ...
        final = None
        final_list = []
        for sample in self._distance_guidance_reverse_diffusion_step(images):
            final = sample
            if not only_final:
                final_list.append(final)
