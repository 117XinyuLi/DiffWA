...
class DiffusionProcessDDPM():
    ...
    def _conditional_one_diffusion_step(self, img, init_y):
        y = init_y
    ...
    @torch.no_grad()
    def N_conditional_sampling(self, img, init_y, only_final=False):
        sampling_list = []

        final = None
        self.diffusion_fn.eval()
        for sample in self._conditional_one_diffusion_step(img, init_y):
            final = sample
            if not only_final:
    ...
    def _one_diffusion_step(self, x, init_y, mode='SSIM'):
        img = x
        total_step = N
        self.diffusion_fn.eval()
        x = init_y
        for idx in reversed(range(total_step)):
            ...
            if mode == 'SSIM':
                gama = 1/255
                a = 1
                s_t = 3 * torch.sqrt(1 - self.alpha_bars[idx]) * a / (torch.sqrt(self.alpha_bars[idx]) * gama)
    ...
    @torch.no_grad()
    def N_distance_guidance_sampling(self, img, init_y, only_final=False):
        sampling_list = []

        final = None
        self.diffusion_fn.eval()
        for sample in self._one_diffusion_step(img, init_y):
            final = sample
            if not only_final:
                sampling_list.append(final)
